    ...
    def filter_high_quality_signals(signals, data_manager):
        ...
        for signal in signals:
            ...
            try:
                ...
                if volume_ratio < 1.1:  # Minimum 30% above average volume
                    continue
                ...
                # 5. Confidence Threshold (minimum 55%)
...
class MultiStrategyIntradayTrader:
    ...
    def generate_high_accuracy_signals(self, symbol, data):
        ...
        try:
            ...
            if (ema8 > ema21 > ema50 and 
                ...
                if rr >= 1.3:  # Higher risk-reward for high accuracy
                    signals.append({
                        "symbol": symbol,
                        "action": action,
                        "entry": current_price,
    ...
    def generate_strategy_signals(self, symbol, data):
        ...
        try:
            ...
            if (ema8 > ema21 > ema50 and live > vwap and adx_val > 20 and htf_trend == 1):
                ...
                if rr >= 1.3:  # Increased from 2.0
                    ...
                    if historical_accuracy >= 0.65:
                        ...
                        # Volume confirmation
                        if vol_latest < vol_avg * 1.1:  # Require 30% above average volume
            ...
            if live <= float(data["BB_Lower"].iloc[-1]) if "BB_Lower" in data.columns else False:
                action = "BUY"; confidence = 0.55; score = 7; strategy = "Bollinger_Reversion"
                target, stop_loss = self.calculate_improved_stop_target(live, action, atr, live, support, resistance)
                rr = abs(target - live) / max(abs(live - stop_loss), 1e-6)
                if rr >= 1.3:
                    historical_accuracy = self.data_manager.get_historical_accuracy(symbol, strategy)
            ...
            if live >= float(data["BB_Upper"].iloc[-1]) if "BB_Upper" in data.columns else False:
                action = "SELL"; confidence = 0.55; score = 7; strategy = "Bollinger_Rejection"
                target, stop_loss = self.calculate_improved_stop_target(live, action, atr, live, support, resistance)
                rr = abs(target - live) / max(abs(live - stop_loss), 1e-6)
                if rr >= 1.3:
                    historical_accuracy = self.data_manager.get_historical_accuracy(symbol, strategy)
            ...
            if ((rsi_val > 70 and rsi_val < data["RSI14"].iloc[-2]) or 
                ...
                confidence = 0.55; score = 7; strategy = "Trend_Reversal"
                target, stop_loss = self.calculate_improved_stop_target(live, action, atr, live, support, resistance)
                rr = abs(target - live) / max(abs(live - stop_loss), 1e-6)
                if rr >= 1.3:
                    historical_accuracy = self.data_manager.get_historical_accuracy(symbol, strategy)
    ...
    def generate_quality_signals(self, universe, max_scan=None, min_confidence=0.55, min_score=4, use_high_accuracy=True):
        signals = []
...
try:
    ...
    with quality_cols[0]:
        ...
        <div class="high-quality-signal">
            ...
            <div style="font-size: 12px; margin-top: 5px;">• RR ≥ 1.3:1</div>
            <div style="font-size: 12px;">• Volume ≥ 1.1x</div>
            <div style="font-size: 12px;">• Confidence ≥ 55%</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tabs[9]:
        ...
        with col2:
            min_high_acc_confidence = st.slider("Min Confidence", 40, 90, 55, 5, key="high_acc_conf")
        with col3:
