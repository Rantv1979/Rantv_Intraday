...
# ------------------------
# BEGIN RUNTIME PATCH (auto-appended by assistant)
# Purpose: apply top fixes (normalized caching, robust equity stats,
# safer Yahoo fetch, safe df updates, minimum-sample guard for historical accuracy)
# This block monkey-patches the classes defined earlier in this file at import/runtime.
# ------------------------
import time
import math
...
def _patched_fetch_yf(symbol: str, period: str, interval: str):
    """Robust wrapper around yf.download with a light retry. Returns DataFrame or empty DataFrame."""
    attempts = 2
    last_exc = None
    for _ in range(attempts):
        try:
            df = _yf.download(symbol, period=period, interval=interval, progress=False, auto_adjust=False)
            if df is None:
...
def _patched_compute_equity_curve_stats(self, equity_series):
    """Robust equity stats computation used by RealBacktestEngine."""
    try:
        eq = _pd.Series(equity_series).dropna().astype(float)
        ...
        returns = _pd.Series(cum).pct_change().dropna()
        if returns.empty:
            return {"max_drawdown": max_dd, "max_drawdown_pct": max_dd_pct, "sharpe": 0.0, "total_return": total_return}

        mean_r = float(returns.mean())
        std_r = float(returns.std(ddof=1))
        if std_r <= 0 or _np.isnan(std_r):
            sharpe = 0.0
        else:
...
def _patched_calculate_historical_accuracy(self, symbol, strategy, data, min_trades:int=20):
    """
    Normalized calculate_historical_accuracy: always caches a structured dict and enforces a minimum sample size.
    Returns a scalar win_rate (for backwards compatibility).
    """
    key = f"{symbol}_{strategy}"
    if key in self.historical_results:
        cached = self.historical_results[key]
    ...
    # fallback simple generator (conservative)
    def fallback_generator(d_slice):
        if len(d_slice) < 20:
            return None
        try:
            ...
            if strategy == "EMA_VWAP_Confluence" and ema8 > ema21 > ema50 and close > vwap and adx_val > 20:
                return {"action": "BUY"}
            if strategy == "RSI_MeanReversion" and rsi_val < 30:
                return {"action": "BUY"}
            if strategy == "Bollinger_Reversion" and "BB_Lower" in d_slice.columns and close <= d_slice["BB_Lower"].iloc[-1]:
                return {"action": "BUY"}
            if strategy == "MACD_Momentum" and macd_line > macd_signal and ema8 > ema21 and close > vwap:
                return {"action": "BUY"}
            if strategy == "EMA_VWAP_Downtrend" and ema8 < ema21 < ema50 and close < vwap:
                return {"action": "SELL"}
            if strategy == "RSI_Overbought" and rsi_val > 70:
                return {"action": "SELL"}
            return None
        except Exception:
            return None
    ...
    try:
        result = self.backtest_with_generator(data, fallback_generator,
                                              initial_capital=1_000_000.0,
                                              risk_per_trade=0.01,
                                              cost_pct=0.0004,
                                              slippage_pct=0.0003,
        ...
        total_trades = int(result_metrics.get("total_trades", 0))
        if total_trades < min_trades:
            normalized = {"trades": result.get("trades", []), "metrics": result_metrics, "note": f"insufficient_trades<{min_trades}"}
            self.historical_results[key] = normalized
            return 0.0
        normalized = {"trades": result.get("trades", []), "metrics": result_metrics}
        self.historical_results[key] = normalized
        return float(result_metrics.get("win_rate", 0.0))
    except Exception as e:
        self.historical_results[key] = {"trades": [], "metrics": {}, "note": f"error:{str(e)}"}
        return 0.0

# Try to apply monkey-patches if the classes are defined in the current namespace (they should be from the original file)
try:
    if 'RealBacktestEngine' in globals():
        RealBacktestEngine._compute_equity_curve_stats = _patched_compute_equity_curve_stats
        RealBacktestEngine.calculate_historical_accuracy = _patched_calculate_historical_accuracy
    if 'EnhancedDataManager' in globals():
        # Attach fetch wrapper and note: the original EnhancedDataManager may already have _fetch_yf; replace safely
        EnhancedDataManager._fetch_yf = staticmethod(_patched_fetch_yf)

        # Replace get_stock_data with a safer wrapper that calls original then does safe updates where needed.
        _orig_get_stock_data = EnhancedDataManager.get_stock_data
        def _safe_get_stock_data(self, symbol, interval="15m"):
            df = _orig_get_stock_data(self, symbol, interval)
            try:
                # if df is DataFrame and has expected columns, ensure last-row safe updates use .at
                if isinstance(df, _pd.DataFrame) and not df.empty and set(["Open","High","Low","Close","Volume"]).issubset(df.columns):
                    # validate live price and adjust last row safely
                    try:
                        live_price = self._validate_live_price(symbol)
                        last_idx = df.index[-1]
                        current_close = float(df.at[last_idx, "Close"])
                        price_diff_pct = abs(live_price - current_close) / max(current_close, 1e-9)
                        if price_diff_pct > 0.005:
                ...
                return df
        EnhancedDataManager.get_stock_data = _safe_get_stock_data
except Exception:
    pass

# End of runtime patch
# ------------------------
