...
<div class="ticker-wrap">
    <div class="ticker-content">
        <span class="ticker-item">NIFTY 50: {data_feed.get_live_price('^NSEI'):,.2f}</span>
        <span class="ticker-item">BANKNIFTY: {data_feed.get_live_price('^NSEBANK'):,.2f}</span>
...
# 2. HEADER
st.markdown('<div class="terminal-header">RANTV INSTITUTIONAL TERMINAL <span style="font-size: 12px; color: #666;">| V.2.1.0 BLACKBOX</span></div>', unsafe_allow_html=True)
    ...
    with c1:
        ...
        if curr_time - st.session_state.last_signal_time > INSTITUTIONAL_CONFIG['SIGNAL_REFRESH_SEC']:
            with st.spinner("Scanning Market Microstructure (Live)..."):
                new_signals = []
                targets = NIFTY_50[:20] # Scan top 20 for speed
                    ...
                    if df is not None:
                        ...
                        if abs(score) >= 3:
                            ...
                            new_signals.append({
                                ...
                                "LTP": ltp,
                                "Signal": signal_type,
                                "Strategy": "MeanRev" if "RSI" in reason[0] else "Trend",
...
# === TAB 5: CHARTS ===
with tabs[4]:
    chart_sym = st.selectbox("Select Asset", NIFTY_50, key="chart_sel")
